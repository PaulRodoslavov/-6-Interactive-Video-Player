...
myVideo.addEventListener("timeupdate", () => {
	...
	for (let i = 0; i < text.length; i++){				
		if(myVideo.currentTime > text[i].getAttribute('data-start') && myVideo.currentTime <= text[i].getAttribute('data-end')){
			text[i].classList.add("orangeColorText");
		}
		else {
			text[i].classList.remove("orangeColorText");
		}
	}
});
...
warpperText.addEventListener('click',(e) => {
	if (e.target.tagName == ('SPAN')){
		myVideo.currentTime = +e.target.getAttribute('data-start') + .1; // after adding 0.1sec only one subtitle is orange		 		 
		myVideo.play();	
	}		 
});
